...
/// <summary>
/// This is the main class of our project. It derives from gvk::invokee and as such gets invoked by the framework each
/// update and render-step.
/// </summary>
...
public:
	...
	/// </summary>
	/// <param name="filename">The full filename of the .obj dataset file</param>
	...
	/// </summary>
	void resetCamera() {
		mQuakeCam->set_translation({ 0.9f, 0.9f, 0.9f });
	...
	bool inCameraMode() {
		...
		if (nullptr != imguiManager) {
			return !imguiManager->is_user_interaction_enabled();
		}
		return false;
	}
	...
	/// <summary>
	/// This function gets called by the framework before the first iteration of the render-loop.
	/// It initializes all of our needed buffers, textures, renderpasses and pipelines.
	/// </summary>
	void initialize() override
	{
		...
		mPipeline = context().create_graphics_pipeline_for(
			from_buffer_binding(0)->stream_per_vertex(&Vertex::position)->to_location(0),
			from_buffer_binding(0)->stream_per_vertex(&Vertex::curvature)->to_location(1),
			from_buffer_binding(0)->stream_per_vertex(&Vertex::data)->to_location(2),
		...
		if (nullptr != imguiManager) {
			...
			imguiManager->add_callback([this] {renderUI(); });
		}
	...
	void update() override
	{
		...
		if (gvk::input().key_pressed(key_code::f1))
			toggleInputMode(); // F1 toggles between "quake_cam mode" and "UI mode":
		if (gvk::input().key_pressed(key_code::f2)) 
			mShowUI = !mShowUI; // F2 hides/shows the UI
		if (gvk::input().key_pressed(key_code::f3))
			mShowStatisticsWindow = !mShowStatisticsWindow; // F2 hides/shows the statistic window
		if (gvk::input().key_pressed(key_code::f11)) {
			toggleFullscreenMode();
		}
	...
	void render() override
	{
		...
		if (mMainRenderPassEnabled && mVertexBuffer.has_value()) {
			...
			for (unsigned int i = 0; i < mDrawCalls.size(); i++) {
				...
				cmdBfr->draw_vertices(mDrawCalls[i].numberOfPrimitives, 1u, mDrawCalls[i].firstIndex, 1u, const_referenced(mVertexBuffer));
			}
			cmdBfr->end_render_pass();
		}
